...
def _default_hooks(project_slug: str) -> HooksConfig:
    ...
    return HooksConfig(
        ...
        after_run=(
            ...
            "  echo \"Execution workspace is still on ${BASE_BRANCH}; expected an issue branch.\" >&2\n"
            "  exit 1\n"
            "fi\n"
            "\n"
            "ISSUE_ID=$(basename \"$PWD\" | tr '[:lower:]' '[:upper:]')\n"
            "if ! git diff --quiet || ! git diff --cached --quiet; then\n"
            "  git add -A\n"
            "  if ! git diff --cached --quiet; then\n"
            "    git commit -m \"${ISSUE_ID}: agent work [skip ci]\"\n"
            "  fi\n"
            "fi"
            "\n"
            "git push -u origin \"$BRANCH\" --force-with-lease || true\n"
            "TITLE=$(git log --format=\"%s\" \"origin/$BASE_BRANCH..HEAD\" | tail -1)\n"
            "if gh pr view >/dev/null 2>&1; then\n"
            "  gh pr edit --title \"$TITLE\" || true\n"
            "else\n"
            "  gh pr create --title \"$TITLE\" --body \"\" --base \"$BASE_BRANCH\" --head \"$BRANCH\" || true\n"
            "fi"
        ),
        before_remove=None,
